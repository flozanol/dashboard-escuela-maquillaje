          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mesesCobranza.map(mes => ({
                ...
                <CartesianGrid strokeDasharray="3 import React, { useState, useMemo, useEffect } from 'react';
...
const Dashboard = () => {
  ...
  const fetchGoogleSheetsData = async (showLoading = true) => {
    ...
    try {
      ...
      console.log('🔄 Datos actualizados desde Google Sheets:', {
        ventasKeys: Object.keys(transformedVentas),
        contactKeys: Object.keys(transformedContact),
        timestamp: new Date().toISOString()
      });
      
      // Forzar actualización completa de datos
      setSalesData(prev => {
        console.log('📊 Actualizando salesData - anterior:', Object.keys(prev));
        console.log('📊 Actualizando salesData - nuevo:', Object.keys(transformedVentas));
        return { ...transformedVentas };
      });
      
      setContactData(prev => {
        console.log('📞 Actualizando contactData');
  ...
  // Agregar clave de dependencia para forzar recálculo cuando los datos cambien
  const dataHash = useMemo(() => {
    return JSON.stringify(Object.keys(salesData)) + Object.values(salesData).length;
  }, [salesData]);

  const schools = useMemo(() => {
    console.log('🏫 Recalculando schools con dataHash:', dataHash);
    ...
    const result = Array.from(schoolsSet);
    console.log('🏫 Schools encontradas:', result);
    return result;
  }, [salesData, dataHash]);

  const areas = useMemo(() => {
    console.log('📚 Recalculando areas con dataHash:', dataHash);
    ...
    const result = Array.from(areasSet);
    console.log('📚 Areas encontradas:', result);
    return result;
  }, [salesData, dataHash]);

  const instructors = useMemo(() => {
    console.log('👨‍🏫 Recalculando instructors con dataHash:', dataHash);
    ...
    const result = Array.from(instructorsSet);
    console.log('👨‍🏫 Instructors encontrados:', result);
    return result;
  }, [salesData, dataHash]);

  const months = useMemo(() => {
    const result = Object.keys(salesData).sort();
    console.log('📅 Meses recalculados:', result);
    return result;
  }, [salesData, dataHash]);

  // Nuevo computed para medios de contacto
  const contactMethods = useMemo(() => {
    console.log('📞 Recalculando contactMethods');
    ...
    const result = Array.from(methodsSet);
    console.log('📞 Contact methods encontrados:', result);
    return result;
  }, [contactData, dataHash]);
  ...
  const getViewData = useMemo(() => {
    console.log('🔄 Recalculando getViewData para viewType:', viewType);
    console.log('📊 Con salesData keys:', Object.keys(salesData));
    console.log('👥 Instructors disponibles:', instructors);
  ...
  }, [viewType, selectedMonth, selectedSchool, selectedArea, metricType, months, schools, compareMonths, contactData, salesData, instructors, areas, dataHash]);
