...
export default Dashboard;  // Nuevo componente para el dashboard de medios de contacto
  ...
  const CobranzaDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* NUEVA SECCIÓN: Tabla de Ingresos */}
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-800">Tabla de Ingresos Detallada</h3>
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <DollarSign className="w-4 h-4" />
              <span>{ingresosData.length} conceptos registrados</span>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  .map((item, index) => {
                    ...
                    return (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            porcentaje > 30 ? 'bg-green-100 text-green-800' :
                            porcentaje > 15 ? 'bg-yellow-100 text-yellow-800' :
                            porcentaje > 5 ? 'bg-orange-100 text-orange-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                            {porcentaje > 30 ? 'Principal' :
                             porcentaje > 15 ? 'Importante' :
                             porcentaje > 5 ? 'Moderado' :
                             'Menor'}
                          </span>
                        </td>
                      </tr>
                ...
                <tr className="bg-gradient-to-r from-green-100 to-green-200 font-bold border-t-2 border-green-300">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-green-900 font-bold">
                    ${ingresosData.reduce((sum, item) => sum + item.monto, 0).toLocaleString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-green-900 font-bold">
                    100.0%
                  </td>
        ...
        {/* NUEVA SECCIÓN: Gráfico de Ingresos por Concepto */}
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Gráfico de Barras */}
            <div className="h-80">
              <h4 className="text-md font-medium mb-2 text-gray-700">Comparación por Monto</h4>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={ingresosData.sort((a, b) => b.monto - a.monto)} layout="horizontal">
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis type="number" tickFormatter={(value) => `${(value/1000).toFixed(0)}k`} />
                  <YAxis dataKey="concepto" type="category" width={120} fontSize={12} />
            ...
            {/* Gráfico de Pastel */}
            <div className="h-80">
              <h4 className="text-md font-medium mb-2 text-gray-700">Distribución Porcentual</h4>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={ingresosData.map(item => {
                      ...
                      return {
                        name: item.concepto,
                        value: item.monto,
                        percentage: ((item.monto / totalIngresos) * 100).toFixed(1)
                      };
                    })}
                    cx="50%"
                    cy="50%"
                  ...
                  >
                    {ingresosData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={['#22C55E', '#3B82F6', '#8B5CF6', '#F59E0B', '#EF4444', '#06B6D4', '#EC4899'][index % 7]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(value) => [`${value.toLocaleString()}`, 'Monto']} />
                  <Legend />
...
// Datos de fallback para tabla de ingresos
const fallbackIngresosData = [
  { concepto: "Ventas Totales", monto: 150000 },
  { concepto: "Cursos Online", monto: 75000 },
  { concepto: "Certificaciones", monto: 45000 },
  { concepto: "Maquillaje Profesional", monto: 30000 }
];
...
const Dashboard = () => {
    ...
    try {
      ...
      try {
        ...
        if (ingresosResponse.ok) {
          ...
          console.warn('No se pudieron cargar datos de ingresos, usando fallback');
          setIngresosData(fallbackIngresosData);
        }
      } catch (ingresosError) {
        console.warn('Error cargando datos de ingresos:', ingresosError);
        setIngresosData(fallbackIngresosData);
      }
  ...
  const transformIngresosData = (rawData) => {
    ...
    // Saltar la fila de headers si existe
    const startIndex = rawData[0] && (rawData[0][0]?.toLowerCase().includes('concepto') || rawData[0][0]?.toLowerCase().includes('descripción')) ? 1 : 0;
    
    for (let i = startIndex; i < rawData.length; i++) {
      const row = rawData[i];
      if (row && row.length >= 2 && row[0] && row[1]) {
        const concepto = row[0].toString().trim();
        const monto = parseNumberFromString(row[1]);
    ...
    return transformedData.length > 0 ? transformedData : fallbackIngresosData;
  };
