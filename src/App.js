...
console.log('\n✅ Resultado final de transformación:', result);
  ...
  const transformIngresosData = (rawData) => {
    ...
    rows.forEach(row => {
      const [medio, monto] = row;
      if (medio && medio.trim() !== '') {
        const medioClean = medio.trim();
        const montoNum = parseNumberFromString(monto);
        
        if (transformedData[medioClean]) {
          transformedData[medioClean] += montoNum;
        } else {
          transformedData[medioClean] = montoNum;
        }
      }
...
const escuelas = Object.keys(cobranzaData);
    ...
    const ingresosDetails = useMemo(() => {
      if (!ingresosData || Object.keys(ingresosData).length === 0) {
        return { tableData: [], pieData: [], total: 0 };
      }
      
      const total = Object.values(ingresosData).reduce((sum, val) => sum + val, 0);
      
      const tableData = Object.entries(ingresosData)
        .map(([medio, monto]) => ({
          medio,
          monto,
      ...
      const pieData = tableData.map(({ medio, monto }) => ({
        name: medio,
        value: monto
      }));
      
      return { tableData, pieData, total };
    }, [ingresosData]);
...
0
     ...
     textAnchor="end"
        ...
        {/* Análisis de Ingresos por Medio */}
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          {/* Tabla de Ingresos */}
          <div className="lg:col-span-3 bg-white rounded-lg shadow-lg p-6">
            <h3 className="text-lg font-semibold mb-4">Ingresos por Medio</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Medio de Ingreso</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {ingresosDetails.tableData.map(({ medio, monto, porcentaje }) => (
                    <tr key={medio} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{medio}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">${monto.toLocaleString()}</td>
                ...
                <tfoot className="bg-gray-100 font-bold">
                    <tr>
                        <td className="px-6 py-3 text-left text-sm">Total</td>
                        <td className="px-6 py-3 text-left text-sm">${ingresosDetails.total.toLocaleString()}</td>
                        <td className="px-6 py-3 text-left text-sm">100%</td>
                    </tr>
                </tfoot>
              </table>
            </div>
          ...
          {/* Gráfico de Ingresos */}
          <div className="lg:col-span-2 bg-white rounded-lg shadow-lg p-6 flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-4 w-full">Distribución de Ingresos</h3>
            <div className="w-full h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={ingresosDetails.pieData}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
                    outerRadius={100}
                  ...
                  >
                    {ingresosDetails.pieData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
     ...
     <img 
                ...
                onError={(e) => {
                  // Fallback en 
caso de que la imagen no cargue
