...
if (Object.keys(salesData).length === 0) {
  ...
  const transformGoogleSheetsData = (rawData) => {
    ...
    rows.forEach((row, index) => {
      ...
      // Limpiar el nombre del instructor y asignar un valor por defecto si está vacío
      const instructor = instructorRaw ? String(instructorRaw).trim() : 'No asignado';
      ...
      if (transformedData[monthKey][escuela][area][curso]) {
        ...
        transformedData[monthKey][escuela][area][curso] = {
          ventas: ventasNum,
          cursos: cursosNum,
          instructor: instructor // El instructor ya está limpio y con valor por defecto
        };
      }
    });
