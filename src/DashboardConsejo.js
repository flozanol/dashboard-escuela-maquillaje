...
function normalizeDateInput(input) {
  ...
  if (/^\d+(?:\.\d+)?$/.test(raw)) {
    ...
    return new Date(excelEpoch.getTime() + days * 86400000).toISOString().substring(0, 10);
  }
...
function processVentas(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const fecha = normalizeDateInput(row[0]);
    if (!fecha) continue;
...
function processPolancoIngresos(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // ✅ Columna K = índice 10, acepta POLANCO y POLANO (typo en Sheets)
    const campus = (row[10] ?? '').toString().trim().toUpperCase();
    const esPolanco = campus === 'POLANCO' || campus === 'POLANO';
    if (!fecha || !esPolanco || ingreso <= 0) continue;
    const mes = fecha.substring(0, 7);
...
export default function DashboardConsejo() {
  ...
  const toggleMonth = mes => setSelectedRangeMonths(prev =>
  ...
  const getSedeData = (sedeObj, sedeName, month) => {
    ...
    const obj = objetivos[`${month}-${sedeName}`] ??
      objetivos[`${month}-${sedeName === 'QRO' ? 'QUERÉTARO' : sedeName}`] ??
      { ventas: 0, cursos: 0 };
    return {
      actual, obj,
  ...
  const dataGraficas = mesesDisponibles.slice().reverse().map(m => ({
    ...
    CDMXC: data.cdmx.porMes[m]?.cursos ?? 0,
    QROC: data.qro.porMes[m]?.cursos ?? 0,
    OnlineC: data.online.porMes[m]?.cursos ?? 0,
  }));
  ...
  const
    CardSede = ({ titulo, info, color, icon: IconComponent }) => {
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex flex-col items-end">
            <div className="flex items-center gap-1">
              {info.crecimiento >= 0
                ? <TrendingUp size={16} color={IDIP_GREEN} />
                : <TrendingDown size={16} color={COLOR_RED} />}
            ...
            <span style={{ fontSize: '9px' }} className="font-bold text-gray-400">vs {info.anoAnteriorLabel}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <div className="flex justify-between mb-1" style={{ fontSize: '10px' }}>
              <span className="font-bold text-gray-500 uppercase">Ventas ${info.actual.ventas.toLocaleString()}</span>
              <span className="font-bold text-gray-500">Meta ${info.obj.ventas.toLocaleString()}</span>
            </div>
            <div className="w-full h-2 rounded-full overflow-hidden" style={{ backgroundColor: IDIP_LIGHT_GRAY }}>
              <div className="h-full transition-all duration-700"
                style={{ width: `${Math.min(avanceV, 100)}%`, backgroundColor: getProgressColor(avanceV) }} />
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between mb-1" style={{ fontSize: '10px' }}>
              <div className="flex items-center gap-1">
                <Users size={10} />
                <span className="font-bold text-gray-500 uppercase">Alumnos {info.actual.cursos}</span>
              </div>
              <span className="font-bold text-gray-500">Meta {info.obj.cursos}</span>
            </div>
            <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: IDIP_LIGHT_GRAY }}>
              <div className="h-full transition-all duration-700"
                style={{ width: `${Math.min(avanceC, 100)}%`, backgroundColor: IDIP_GRAY }} />
            </div>
          </div>
            ...
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 space-y-3">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                  ...
                  <span style={{ fontSize: '10px' }} className="font-black text-gray-600 uppercase">Ritmo / Pace</span>
                </div>
                <span style={{ fontSize: '10px' }}
                  className={`font-black px-2 py-0.5 rounded-full ${pace >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                  ...
                  <span style={{ fontSize: '10px' }} className="font-black text-gray-600 uppercase">Pronóstico</span>
                </div>
                <div className="text-right">
                  <p className="text-xs font-black text-gray-800">${Math.round(forecast).toLocaleString()}</p>
                  <p style={{ fontSize: '9px' }} className="font-bold text-gray-400">Proj {forecastPercent.toFixed(1)}%</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="pt-3 border-t border-gray-100 flex justify-between items-center">
            <span style={{ fontSize: '9px' }} className="font-bold text-gray-400 uppercase">Alcance vs Año Anterior</span>
            <div className="text-right">
              <p style={{ fontSize: '9px', color: alcanceAnual >= 100 ? IDIP_GREEN : IDIP_GRAY }} className="font-bold">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-2xl shadow-sm gap-4 border-b-2" style={{ borderBottomColor: IDIP_GREEN }}>
          ...
          <div className="flex items-center gap-3">
            <a href="https://idip-dashboard.vercel.app" target="_blank" rel="noopener noreferrer"
              ...
              <ExternalLink size={14} /> Dashboard Marketing
            </a>
            <div className="flex items-center gap-3 bg-gray-50 p-2 px-4 rounded-xl border border-gray-100">
              ...
              <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)}
                className="bg-transparent font-bold outline-none cursor-pointer text-sm" style={{ color: IDIP_GRAY }}>
        ...
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div style={{ fontSize: '10px' }} className="font-bold text-gray-400 uppercase">{selectedRangeMonths.length} meses seleccionados</div>
          </div>

          <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1 border-b border-gray-50 pb-4">
            {mesesDisponibles.map(mes => (
              <button key={mes} onClick={() => toggleMonth(mes)}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-xs font-black uppercase
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Suma Ventas', value: rangeData.total, color: IDIP_GRAY },
              { label: 'Suma CDMX', value: rangeData.cdmx, color: IDIP_GREEN },
              { label: 'Suma QRO', value: rangeData.qro, color: IDIP_GRAY },
              { label: 'Suma Online', value: rangeData.online, color: IDIP_GREEN },
            ].map(({ label, value, color }) => (
              <div key={label} className="p-6 rounded-2xl text-white shadow-lg" style={{ backgroundColor: color }}>
                <p style={{ fontSize: '9px' }} className="opacity-80 uppercase font-black tracking-widest">{label}</p>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dataGraficas}>
                  ...
                  <Bar name="CDMX" dataKey="CDMXC" fill={IDIP_GRAY} radius={[4, 4, 0, 0]} />
                  <Bar name="QRO" dataKey="QROC" fill={IDIP_GREEN} radius={[4, 4, 0, 0]} />
                  <Bar name="Online" dataKey="OnlineC" fill="#A6A8AB" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
                ...
                <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-black text-lg uppercase tracking-tight" style={{ color: IDIP_GRAY }}>Polanco Ingresos</h2>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      { label: 'Ingreso Mes', value: currentPolanco, color: IDIP_GRAY },
                      { label: 'Acum. Rango', value: polancoRangeTotal, color: IDIP_GREEN },
                      { label: '% vs CDMX', value: `${polancoVsCDMX.toFixed(1)}%`, color: IDIP_GRAY },
                      { label: 'Ticket Prom.', value: `$${Math.round(polancoTicketPromedio).toLocaleString()}`, color: IDIP_GREEN },
                    ].map(({ label, value, color }) => (
                      <div key={label} className="p-6 rounded-2xl text-white shadow-lg" style={{ backgroundColor: color }}>
                        <p style={{ fontSize: '9px' }} className="opacity-80 uppercase font-black tracking-widest">{label}</p>
                        <p className="text-2xl font-black mt-1">{typeof value === 'string' ? value : `$${value.toLocaleString()}`}</p>
                      </div>
                    ))}
                  </div>
                </section>
                ...
                <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
                  <div className="flex items-center gap-2">
                    <Award size={22} color={IDIP_GREEN} />
                    <h2 className="font-black text-lg uppercase tracking-tight" style={{ color: IDIP_GRAY }}>Top 10 Escuelas</h2>
                  </div>
                  <div className="space-y-3">
                    {rankingEscuelas.map((esc, idx) => (
                      <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-100">
                        <span className="font-bold text-gray-700">{idx + 1}. {esc.n}</span>
                        <span className="font-black text-lg" style={{ color: IDIP_GREEN }}>${esc.v.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </section>
